          ...
          <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200/80 dark:border-white/10 bg-white/80 dark:bg-slate-900/80 px-4 sm:px-6 backdrop-blur-xl">
            <div className="flex items-center gap-3">
              <details className="group relative shrink-0 lg:hidden">
                <summary
                  className="grid h-10 w-10 list-none place-items-center rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-200 shadow-2xs marker:content-none"
                  aria-label="Mở menu Staff"
                >
                  <Menu className="h-5 w-5" />
                </summary>
                <nav
                  className="absolute left-0 top-12 z-50 grid w-[min(82vw,18rem)] gap-1 rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 p-2 shadow-2xl"
                  aria-label="Điều hướng Staff"
                >
                  {staffNavItems.map((item) => {
                    const Icon = item.icon;
                    const isActive = activeItem === item.key;
                    return (
                      <Link
                        ...
                        to={item.to}
                        className={cn(
                          "flex min-h-11 items-center gap-3 rounded-xl px-3 py-2 text-xs font-bold",
                          isActive
