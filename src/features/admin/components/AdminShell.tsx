...
const adminNavItems = [
  {
    label: "Dashboard",
    description: "Tổng quan doanh thu",
    to: "/admin/dashboard",
...
function AdminBrand({ collapsed }: { collapsed?: boolean }) {
  return (
    <div
      className={cn(
        "flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3 backdrop-blur-md transition-all duration-300",
...
export function AdminShell({ children }: AdminShellProps) {
  ...
  return (
    <div className="min-h-dvh bg-slate-50 dark:bg-slate-950 text-slate-950 dark:text-slate-100 transition-colors duration-300">
      ...
      <div
        className={cn(
          "grid min-h-dvh transition-all duration-300",
          sidebarCollapsed
      ...
      >
        ...
        <aside className="sticky top-0 hidden h-dvh border-r border-slate-200 bg-white px-4 py-5 text-slate-900 shadow-2xl shadow-slate-950/10 dark:border-transparent dark:bg-slate-950 dark:text-white dark:shadow-slate-950/20 lg:flex lg:flex-col justify-between">
          ...
          <button
            ...
            className="mt-4 flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 text-xs font-bold text-slate-600 hover:bg-slate-100 hover:text-slate-950 transition-colors dark:border-white/10 dark:bg-white/5 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-white"
