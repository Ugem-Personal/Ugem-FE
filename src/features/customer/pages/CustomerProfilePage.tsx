...
function DiningPreferencesCard() {
  ...
  const applyPreferences = (
    ...
    const legacyFoodPreferences = [
      ...
      ...data.preferredRestaurantTypes.filter((value) => value === "Ăn vặt"),
    ];
    const inferredCategoryIds = options.foodCategories
      .filter((category) => {
        ...
        return legacyFoodPreferences.some((preference) => {
          ...
          return ["Cơm", "Bún", "Phở", "Lẩu", "Nướng"].includes(preference)
            ? label === "Món chính"
            : label === preference;
        });
      })
    ...
    setPreferences({
      ...
      preferredMainDishTypes: [],
      preferredCategoryIds:
        savedCategoryIds.length > 0 ? savedCategoryIds : inferredCategoryIds,
  ...
  useEffect(() => {
    let active = true;
    const load = async () => {
      const [preferencesResult, optionsResult] = await Promise.allSettled([
        getCustomerPreferences(),
        getDiscoveryOptions(),
      ]);
      if (!active) return;
      ...
      if (preferencesResult.status === "fulfilled") {
        ...
        applyPreferences(preferencesResult.value, options);
      } else {
        console.error(preferencesResult.reason);
  ...
  const handleReloadData = async () => {
    ...
    if (optionsResult.status === "fulfilled") setDiscoveryOptions(options);
    if (preferencesResult.status === "fulfilled") {
      setPreferencesUnavailable(false);
      applyPreferences(preferencesResult.value, options);
    } else {
    ...
    if (
      ...
      notify.error("Chưa thể tải danh mục món. Vui lòng thử lại sau.");
    }
    ...
    <div className="mt-6 rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white/80 dark:bg-slate-900/60 p-8 shadow-xl backdrop-blur-2xl transition-colors duration-300">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button
          type="button"
          onClick={handleSave}
          disabled={isSaving || isLoading || preferencesUnavailable}
      ...
      {preferencesUnavailable ? (
        <div
          role="alert"
          className="mb-6 flex flex-col gap-3 rounded-2xl border border-rose-500/20 bg-rose-500/5 p-4 sm:flex-row sm:items-center sm:justify-between"
        >
          <div className="flex items-start gap-3">
            <AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-rose-500" />
            <div>
              <p className="text-sm font-bold text-slate-800 dark:text-slate-200">
                Chưa tải được sở thích đã lưu
              </p>
              <p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">
                Tạm thời chưa thể chỉnh sửa để tránh ghi đè dữ liệu của bạn.
              </p>
            </div>
          </div>
          <button
            ...
            className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl border border-rose-500/30 px-4 text-xs font-bold text-rose-600 transition hover:bg-rose-500/10 disabled:cursor-not-allowed disabled:opacity-50 dark:text-rose-400"
          >
            <RefreshCw
              className={`h-3.5 w-3.5 ${isReloadingCategories ? "animate-spin" : ""}`}
            />
            Tải lại
          </button>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="mb-3 text-xs leading-5 text-slate-500 dark:text-slate-400">
              Chọn những nhóm món bạn thường muốn ăn.
            </p>
            {discoveryOptions.foodCategories.length === 0 ? (
              <div
                role="status"
              ...
              >
                ...
                <button
                  ...
                  onClick={handleReloadData}
                  disabled={isReloadingCategories}
                  className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl border border-amber-500/30 px-4 text-xs font-bold text-amber-600 transition hover:bg-amber-500/10 disabled:cursor-not-allowed disabled:opacity-50 dark:text-amber-400"
                >
                  <RefreshCw
                    className={`h-3.5 w-3.5 ${isReloadingCategories ? "animate-spin" : ""}`}
                  />
