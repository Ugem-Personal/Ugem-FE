...
export type IncidentType = "FoodSafety" | "Hygiene" | "Fraud" | "WrongInformation" | "BadService" | "Other";
...
export async function submitMerchantClaim(input: { merchantId: string; evidenceUrls?: string[] }) {
  const response = await api.post("/moderation/claims", input);
  return response.data.data;
}

export async function requestMerchantRemoval(input: { merchantId: string; reason: string }) {
...
export async function disputeCheckIn(checkInId: string, reason?: string) {
  const response = await api.post(`/check-ins/${checkInId}/dispute`, { reason });
  return response.data.data;
}
