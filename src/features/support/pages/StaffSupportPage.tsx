      ...
      <div className="space-y-6">
        <header>
          ...
          <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">
            Tiếp nhận, xử lý và phản hồi các vấn đề từ quán.
          </p>
        </header>
        <div className="grid gap-6 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
          <section className="rounded-3xl border border-slate-200/80 bg-white/80 p-5 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/80">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-black">Ticket cần xử lý</h2>
            </div>
            <div className="mt-4 space-y-3">
              {tickets.length === 0 ? (
                <p className="text-sm text-slate-500">Chưa có ticket.</p>
              ) : (
                tickets.map((ticket) => (
                  <button
                    key={ticket.id}
                    type="button"
                    onClick={() =>
                      void getStaffSupportTicket(ticket.id).then((next) => {
                        setSelected(next);
                        setStatus(next.status);
                      })
                    }
                    className={`w-full rounded-2xl border p-4 text-left transition ${selected?.id === ticket.id ? "border-cyan-500 bg-cyan-50 dark:bg-cyan-950/30" : "border-slate-200 dark:border-white/10"}`}
                  >
          ...
          {selected ? (
            <section className="rounded-3xl border border-slate-200/80 bg-white/80 p-5 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/80">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <select
                    value={status}
                    onChange={(event) => {
                      const next = event.target.value as SupportStatus;
                      setStatus(next);
                      void handleStatus(next);
                    }}
                    className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-xs font-bold dark:border-white/10 dark:bg-slate-950"
                  >
                    <option value="Open">Chờ xử lý</option>
                    <option value="InProgress">Đang xử lý</option>
                    <option value="WaitingForMerchant">
                      Chờ Merchant phản hồi
                    </option>
              ...
              <div className="mt-4 flex items-center gap-2 text-xs text-slate-500">
                ...
                Mọi phản hồi đều được lưu trong ticket và gửi notification.
              </div>
            </section>
          ) : (
            <div className="grid min-h-96 place-items-center rounded-3xl border border-dashed border-slate-200 text-sm text-slate-500 dark:border-white/10">
              <CheckCircle2 className="mb-2 h-6 w-6" />
              Chọn ticket để bắt đầu xử lý.
            </div>
          )}
        </div>
      </div>
